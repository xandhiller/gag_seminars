...
\begin{center}
\begin{tikzpicture}
    \tikzstyle{my_vertex}=[draw, circle, fill=black, minimum size=2pt, inner sep=0pt]
    \draw (0,0) node[my_vertex] (root) {} -- ++(0:4cm) node (end_real_line);
    % dots above zero
    ...
    \filldraw[black] (4.1,0) circle (0.3pt);
    \filldraw[black] (4.2,0) circle (0.3pt);
    \filldraw[black] (4.3,0) circle (0.3pt);
    % Continuation of fourier series
    \filldraw[black] (0,1.8) circle (0.3pt);
    \filldraw[black] (0,1.9) circle (0.3pt);
    \filldraw[black] (0,2.0) circle (0.3pt);
    \filldraw[black] (0,-1.8) circle (0.3pt);
    \filldraw[black] (0,-1.9) circle (0.3pt);
    \filldraw[black] (0,-2.0) circle (0.3pt);
    \node at (-1,0) {$a=0$};
    \node at (5,0) {$a \rightarrow \infty$};
\end{tikzpicture}
\caption{There are two kinds of representations $\mathcal{M}(2)$, the ones produced as a Fourier series by $\mathcal{SO}(2)$, and the infinite dimensional operators. As we will see, $a$ is unique up to rotation along $\mathbb{R}^{+}$. We will leave the Fourier series for someone else and focus on the infinite dimensional operators.}
\end{center}
\end{figure}
...
\section{Results from the Fourier transform on $\mathcal{M}(2)$} 

Finding matrix coefficients of a function:
\[
    \begin{split}
        \hat{f}_{n,m} &= \frac{1}{(2\pi)^{2}} \int^{2\pi}_{0}\int^{2\pi}_{0}\int^{\infty}_{0} f(\rho, \varphi, \theta) (U^{a}_{g^{-1}})_{n,m} \ \rho \ d\rho \ d\varphi \ d\alpha \\
        &= i^{n-m} \left(  \int^{\infty}_{0} F_{n-m,m}(\rho) J_{m-n}(a\rho)  \ \rho \ d\rho   \right) .
    \end{split}
\]

From the inversion formula:
\[
    f(g) = \sum_{m} \sum_{n} e^{-i(m\alpha + (n-m)\varphi)}
    \underbrace{
    \int^{\infty}_{0} 
    \underbrace{
    \left(
...
\[
        ...
        \Rightarrow
        \quad
        \widehat{(f_1 * f_2)} = \hat{f_2}(a) \hat{f_1}(a).
\]

Proofs of all results can be found in the Appendix, and are fairly straight-forward from previous components of the talk.
...
\subsection{Matrix Coefficients of Representations of $\mathcal{M}(2)$} 
\label{sec:m2_reps}
\lstinputlisting[language=Python]{uga_m2.py}


\subsection{Bessel Function} 
\begin{Define}{Integral form of the Bessel function}
